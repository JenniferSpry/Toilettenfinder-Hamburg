...
/**
 * This is the Activity that is always shown when one starts the Application. It
 * shows a SplashScreen with the bf-hh Logo and their website URL. In the
 * background, this Activity starts the LocationUpdateService and
 * POIUpdateService and registers a receiver for their broadcasts. Once both
 * broadcasts are received the activity will kill itself and start the
 * mapActivity.
 */
...
public class ActivitySplashScreen extends ActivityBase {
	...
	protected void onResume() {
		...
		if (checkPlayServices()) {
			...
			if (status == TagNames.TYPE_NOT_CONNECTED
					&& _oldConnecState != _connecState) {
				// show toast that we are connecting to network
				Toast.makeText(this,
						"Netzwerkverbindung wird hergestellt, bitte warten...",
						Toast.LENGTH_LONG).show();
			} else if (status == TagNames.TYPE_MOBILE
					|| status == TagNames.TYPE_WIFI) {
				onNetworkConnected();
	...
	private BroadcastReceiver receiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (bundle != null) {
				if (action.equals(TagNames.BROADC_LOCATION_NEW)
						|| action.equals(TagNames.BROADC_LOCATION_UPDATED)) { // LocationUpdateService
																				// is
																				// finished
	...
	private ServiceConnection mConnection = new ServiceConnection() {
		...
		public void onServiceConnected(ComponentName className, IBinder binder) {
			...
			if (loc == null) {
				service.updateLocation(60000, 3.0f, 5000, 1.0f);// calls
																// AsyncTask and
																// publishes
																// results
			}
