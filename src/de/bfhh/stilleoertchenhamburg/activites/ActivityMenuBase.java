...
/**
 * Magical Menu providing class because I don't want to copy paste this into
 * every Activity
 * 
 * @author Jenne
 */
...
public class ActivityMenuBase extends ActivityBase {
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		// if user clicks on the app icon (home button), current activity is
		// closed
		...
		case R.id.menu_map:
			...
			// set flag for intent to get activity from stack if it's already
			// running
			map.setFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT);
			startActivity(map);
			return true;
		case R.id.menu_impressum:
			Intent activityImpressum = new Intent(getApplicationContext(),
					ActivityImpressum.class);
			activityImpressum.setFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT);
			startActivity(activityImpressum);
			return true;
		case R.id.menu_toi_list:
			Intent activityToiletList = new Intent(getApplicationContext(),
					ActivityToiletList.class);
