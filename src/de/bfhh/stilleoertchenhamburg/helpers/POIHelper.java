...
/**
 * @author: Steffi Helper class for all POI and POI-list needs
 */
public class POIHelper {
	 ...
	 */
	public static ArrayList<POI> setDistancePOIToUser(ArrayList<POI> poiList,
	...
	public static POI setDistanceSinglePOIToUser(POI poi, double userLat,
			double userLng) {
		Location loc = new Location("");
		loc.setLatitude(userLat);
		loc.setLongitude(userLng);
	 ...
	 */
	public static ArrayList<POI> getClosestPOI(ArrayList<POI> poiList,
			int amount) {
		...
		if (!poiList.isEmpty()) {
			Collections.sort(poiList, new Comparator<POI>() {
				...
				public int compare(POI p1, POI p2) {
					return p1.getDistanceToUserInt()
							- p2.getDistanceToUserInt(); // Ascending
				}
			});
		}

		if (amount > 0 && !poiList.isEmpty()) {
			for (int i = 0; i < amount; i++) { // add ten closes points to other
												// List
	...
	public static MarkerOptions getMarkerOptionsForPOI(POI poi, boolean active) {
		...
		return marker.position(new LatLng(poi.getLat(), poi.getLng()))
				.title(poi.getName()).icon(icon);
	}

	/**
	 * find a POI by it's associated markers id
	 */
	public static POI getPoiByIdReference(
			HashMap<String, Integer> markerPOIIdMap, ArrayList<POI> allPOIList,
			String markerId) {
