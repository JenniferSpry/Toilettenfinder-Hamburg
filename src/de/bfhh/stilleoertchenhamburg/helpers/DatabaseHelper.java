...
/**
 * @author Jenne To reduce httpRequests the data is stored in a SQLite Database
 *         and only fetched from the server if it is older than a certain time
 *         delta.
 * 
 *         To fetch data first check {@link #isDataStillFresh} If false first
 *         fetch data from server and refresh database with
 *         {@link #refreshAllPOI(List, Context)} Once data is fresh use
 *         {@link #getAllPOI()} to get data
 */
public class DatabaseHelper extends SQLiteOpenHelper {
	...
	/**
	 * use this method to get an instance of the class, don't instantiate it
	 * yourself
	 */
	...
	public void onCreate(SQLiteDatabase db) {
		Log.d(TAG, "onCreate");
		final String CREATE_POI_TABLE = "CREATE TABLE IF NOT EXISTS "
				+ TABLE_POIS + "(" + KEY_ID + " INTEGER PRIMARY KEY,"
				+ KEY_NAME + " TEXT," + KEY_ADDRESS + " TEXT," + KEY_DESCR
				+ " TEXT," + KEY_WEBSITE + " TEXT," + KEY_LAT + " REAL,"
				+ KEY_LNG + " REAL," + KEY_CAT_ID + " INTEGER " + ")";
	...
	public boolean isDataStillFresh(Context context) {
		SharedPreferences pref = context.getApplicationContext()
				.getSharedPreferences(PREF_FILE_NAME, 0); // 0 = private mode
		...
		Log.i(TAG, "isDataStillFresh: Last data refresh "
				+ (timeDelta / (1000 * 60)) + " minutes ago");
	...
	public ArrayList<POI> getAllPOI() {
		...
		final String selectQuery = "SELECT * FROM " + TABLE_POIS + " WHERE "
				+ KEY_CAT_ID + " = " + QUERY_CAT_ID + " ORDER BY id DESC";
